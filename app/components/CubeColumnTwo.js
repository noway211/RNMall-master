...
class CubeColumnTwo extends PureComponent {
        ...
        return(
            <View style={{marginTop:-1,flexDirection:'row',alignItems:'center',justifyContent:'space-between',width:width,height:width*cHeight/cWidth}}>
                <TouchableWithoutFeedback onPress={() => AndroidJumpModule.jumpByUrl(JSON.stringify(contents[0].jump))}>
                    <Image source={{uri:resizeImgByWidth(contents[0].image,width/2)}} style={{width:width/2,height:width*cHeight/cWidth}} resizeMode={'cover'} resizeMethod={'resize'}/>
                </TouchableWithoutFeedback>
                <TouchableWithoutFeedback onPress={() => AndroidJumpModule.jumpByUrl(JSON.stringify(contents[1].jump))}>
                    <Image source={{uri:resizeImgByWidth(contents[1].image,width/2)}} style={{width:width/2,height:width*cHeight/cWidth}} resizeMode={'cover'} resizeMethod={'resize'}/>
                </TouchableWithoutFeedback>
            </View>
        );

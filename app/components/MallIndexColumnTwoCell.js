   ...
   render(){
        ...
        <TouchableWithoutFeedback onPress={() => AndroidJumpModule.jumpByUrl(JSON.stringify(content.jump))}>
            <View style={styles.listItemRoot}>
                <Image source={this.state.isOk?{uri:resizeImgByWidth(content.image,(width - 20)/2),cache:'force-cache'}:defaultIcon} style={styles.itemImage} resizeMode={this.state.isOk?'contain':'stretch'} resizeMethod={'resize'} />
